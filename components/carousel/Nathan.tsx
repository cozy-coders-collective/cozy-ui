...
const ROTATE = 45
const ROTATE_ORDER = [0, 45, 50, 55, 60, 65, 70, 75, 80, 85]
const OPACITY_ORDER = [1, 0.8, 0.5, 0.2]
const SCALE_ORDER = [1, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2, 0.1];
...
function Nathan({ imageUrls }: { imageUrls: string[] }) {
  ...
  console.log("current max width!", maxWidth);
  
  return (
    <div className="h-[60vh] mt-12 mb-16 w-full">
      {/* add orange border here on the ref for debugging if needed */}
      <div
        ref={coverflowRef}
        className="relative flex items-center left-1/2 h-full border border-orange-400"
        style={{
      ...
      >
        {imageUrls.map((imageUrl, index) => {
          ...
          const rotate = index > currentIndex ? -ROTATE_ORDER[Math.min(SCALE_ORDER.length - 1, distanceFromMiddle)] : ROTATE_ORDER[Math.min(SCALE_ORDER.length - 1, distanceFromMiddle)]

          const scaledWidth = imageWidth * scale
          return (
            <div
              key={`image-${index}`}
              className={isCurrentImage && `my-active`}
              style={{
                position: `absolute`,
                zIndex: `${zIndex}`,
